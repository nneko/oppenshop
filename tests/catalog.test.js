...
describe('catalog create object returns object', () => {
  beforeAll(async () => {
  ...
  it('should insert a new product into collection product', async () => {
    let item = {insertedCount: 1}
    p.shop = s._id.toString()
    //s.owner = u._id
    //console.log(pr)
    const data = await product.create(p);
    p._id = data.insertedId
    //p = pr
    expect(data).toEqual(expect.objectContaining(item))
  })

  it('should insert a new catalog into collection catalog', async () => {
    let item = {insertedCount: 1}
    let cat = {
      owner: s._id.toString(),
      name: 'Beta Catalog',
      description: 'Beta description ...',
      //displayName: 'Beta Products',
      status: 'active',
    ...
    //s.owner = u._id
    //console.log(pr)
    const data = await catalog.create(cat);
    cat._id = data.insertedId
    c = cat
    expect(data).toEqual(expect.objectContaining(item))
  })

  it('should insert product into a catalog into collection catalog', async () => {
    let item = {modifiedCount: 1}
    let pr = {
      shop: s._id.toString(),
      name: 'Another Beta Product',
      description: 'Another Beta description ...',
      displayName: 'Another Beta Products',
      status: 'active',
      quantity:  0,
      price: generator.roundNumber( 0, 2),
      currency: 'jmd',
    ...
    //s.owner = u._id
    //console.log(pr)
    const data_pr = await product.create(pr);
    pr._id = data_pr.insertedId
    //s.owner = u._id
    //console.log(pr)
    c.products.push(pr._id.toString())
    const data = await catalog.update({_id: c._id},c);
    //cat._id = data.insertedId
    //c = cat
    expect(data).toEqual(expect.objectContaining(item))
  })

  it('should remove product from a catalog into collection catalog', async () => {
    let item = {modifiedCount: 1}
    c.products.pop()
  ...
  // Deletion catalog
  it('should delete catalog from collection catalog', async () => {
    let item = {deletedCount: 1}
    //let d = await shop.read({_id: s._id})
    //console.log(d)
    const data = await catalog.delete({_id: c._id})
    expect(data).toEqual(expect.objectContaining(item))
  })
