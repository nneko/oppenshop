...
shop.isValid = (s) => {
    try {
        if (validator.isNotNull(s) && typeof (s.owner) == 'string' && await user.exists({ _id: db.getObjectId(s.owner) }) && typeof (s.name) == 'string' && validator.isNotNull(s.name) && typeof (s.description) == 'string' && validator.isNotNull(s.description) && s.hasOwnProperty('address') && s.hasOwnProperty('phoneNumber')) {
            return true
        } else {
            return false
        }
